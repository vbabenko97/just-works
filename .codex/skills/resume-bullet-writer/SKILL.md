...
Use numbers only when they are supplied by the user or traceable to a provided source. A strong bullet does not require a number; precise scope, ownership, and outcome are sufficient when no verified metric exists.
...
Do not estimate or infer a metric. Ask the user for a verified figure or mark the claim `[NEEDS INPUT]` in working notes. Use an approximation or range only when the user explicitly confirms that approximation or range.
...
- ✅ Every number or metric is verified; unquantified bullets are allowed
