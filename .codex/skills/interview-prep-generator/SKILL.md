...
Build every STAR detail from the user's resume, supplied evidence, or an explicit user statement. Never fill gaps with plausible context, metrics, causality, ownership, or outcomes. Mark missing story fields `[NEEDS INPUT]` in working notes and ask for them before producing a final script.
